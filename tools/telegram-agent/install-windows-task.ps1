...
$Runner = Join-Path $PSScriptRoot "run-telegram-watcher.ps1"
...
Register-ScheduledTask `
    ...
    -Trigger $Trigger `
    -Settings $Settings `
    -Description "Polls the private Emberlist Telegram bot and queues messages for local agent work." `
    -User $env:USERNAME | Out-Null

Write-Host "Registered scheduled task '$TaskName'. It starts at Windows logon."
Write-Host "Start it now with: Start-ScheduledTask -TaskName '$TaskName'"
Write-Host "Queue/state directory: $RepoRoot\.agent-telegram"
